...
func QueryRoll(c *gin.Context) {
	...
	err = database.Db.Get(&roll, "SELECT `roll` FROM `rolls` WHERE `code`=?", dataFront.Code)
	if err != nil {
		c.JSON(500, gin.H{
			"message": "error",
			"error":   err.Error(),
	...
	var answers []Answer
	err = database.Db.Select(&answers, "SELECT `answer` FROM `answer` WHERE `code`=?", dataFront.Code)
